module 0xd0b2dd565e0f2020d66d581a938e7766b2163db4b8c63410c17578d32b4e9e88::adl_tracker {
    ...
    public fun view_adl_bucket_sizes(p0: object::Object<perp_market::PerpMarket>): (vector<u64>, vector<u64>)
        acquires ADLTracker
    {
        let _v0 = object::object_address<perp_market::PerpMarket>(&p0);
        let _v1 = borrow_global<ADLTracker>(_v0);
        let _v2 = 0x1::vector::empty<u64>();
        let _v3 = 0x1::vector::empty<u64>();
        let _v4 = 0x1::vector::length<big_ordered_map::BigOrderedMap<ADLKey, ADLValue>>(&(&_v1.long_positions).buckets);
        let _v5 = 0;
        let _v6 = false;
        loop {
            ...
            if (!(_v5 < _v4)) break;
            let _v7 = &mut _v2;
            let _v8 = big_ordered_map::compute_length<ADLKey,ADLValue>(0x1::vector::borrow<big_ordered_map::BigOrderedMap<ADLKey, ADLValue>>(&(&_v1.long_positions).buckets, _v5));
            0x1::vector::push_back<u64>(_v7, _v8);
            let _v9 = &mut _v3;
            let _v10 = big_ordered_map::compute_length<ADLKey,ADLValue>(0x1::vector::borrow<big_ordered_map::BigOrderedMap<ADLKey, ADLValue>>(&(&_v1.short_positions).buckets, _v5));
            0x1::vector::push_back<u64>(_v9, _v10);
            continue
        };
        (_v2, _v3)
    }
    public fun view_adl_cutoffs(p0: object::Object<perp_market::PerpMarket>): vector<u8>
        acquires ADLTracker
    {
        let _v0 = object::object_address<perp_market::PerpMarket>(&p0);
        *&(&borrow_global<ADLTracker>(_v0).long_positions).cutoffs
    }
    public fun view_first_n_adl_positions(p0: object::Object<perp_market::PerpMarket>, p1: bool, p2: u64, p3: u64): (vector<ADLKey>, vector<ADLValue>)
        acquires ADLTracker
    {
        let _v0;
        let _v1 = object::object_address<perp_market::PerpMarket>(&p0);
        let _v2 = borrow_global<ADLTracker>(_v1);
        if (p1) _v0 = &_v2.long_positions else _v0 = &_v2.short_positions;
        let _v3 = 0x1::vector::empty<ADLKey>();
        let _v4 = 0x1::vector::empty<ADLValue>();
        let _v5 = 0x1::vector::length<big_ordered_map::BigOrderedMap<ADLKey, ADLValue>>(&_v0.buckets);
        let _v6 = p2 >= _v5;
        'l0: loop {
            if (!_v6) {
                let _v7 = 0x1::vector::borrow<big_ordered_map::BigOrderedMap<ADLKey, ADLValue>>(&_v0.buckets, p2);
                let _v8 = big_ordered_map::internal_new_begin_iter<ADLKey,ADLValue>(_v7);
                _v5 = 0;
                loop {
                    if (big_ordered_map::iter_is_end<ADLKey,ADLValue>(&_v8, _v7)) p1 = false else p1 = _v5 < p3;
                    if (!p1) break 'l0;
                    let _v9 = *big_ordered_map::iter_borrow_key<ADLKey>(&_v8);
                    let _v10 = *big_ordered_map::iter_borrow<ADLKey,ADLValue>(_v8, _v7);
                    0x1::vector::push_back<ADLKey>(&mut _v3, _v9);
                    0x1::vector::push_back<ADLValue>(&mut _v4, _v10);
                    _v8 = big_ordered_map::iter_next<ADLKey,ADLValue>(_v8, _v7);
                    _v5 = _v5 + 1;
                    continue
                }
            };
