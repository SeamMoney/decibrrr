---
source: https://docs.decibel.trade/api-reference/user/get-active-twap-orders
title: get active twap orders
scraped: 2025-11-28T01:20:51.582Z
---

Get active TWAP orders - Decibel[Skip to main content](#content-area)[Decibel home page](/)Search...⌘K
- [Support](https://discord.gg/decibel)

Search...NavigationUserGet active TWAP orders[Welcome](/)[Quick Start](/quickstart/overview)[Architecture](/architecture/perps/perps-contract-overview)[TypeScript SDK](/typescript-sdk/overview)[REST APIs](/api-reference/user/get-account-overview)[WebSocket APIs](/api-reference/websockets/accountoverview)[Transactions](/transactions/overview)##### User
...
Get active TWAP orderscURL

CopyAsk AI```
curl --request GET \
  --url https://api.netna.aptoslabs.com/decibel/api/v1/active_twaps
```
...
User# Get active TWAP orders

Copy pageRetrieve currently active TWAP (time-weighted average price) orders for a specific user.

Copy pageGET/api/v1/active_twapsTry itGet active TWAP orderscURL
...
[​](#parameter-limit)limitintegerdefault:10Maximum number of active TWAPs to return

Required range: `x >= 0`#### Response

200 - application/jsonActive TWAPs retrieved successfully
...
[​](#response-status)statusstringrequired[​](#response-transaction-unix-ms)transaction_unix_msintegerrequired[​](#response-transaction-version)transaction_versionintegerrequiredRequired range: `x >= 0`[Get account overview](/api-reference/user/get-account-overview)[Get delegations](/api-reference/user/get-delegations)⌘I[x](https://x.com/DecibelTrade)[Powered by Mintlify](https://www.mintlify.com?utm_campaign=poweredBy&utm_medium=referral&utm_source=aptoslabs)